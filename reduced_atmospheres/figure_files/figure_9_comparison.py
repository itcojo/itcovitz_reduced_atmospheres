...
def plot_figure_9() -> None:
    """Plot oxygen fugacity of the mantle before and after remixing with the impacted-generated melt phase.
    ...
    handles.append(lines.Line2D([0.], [0.], label='Bulk mantle',
                                color='k', linestyle='-', linewidth=2))
    handles.append(lines.Line2D([0.], [0.], label='Erratum',
    ...
    if errata:
        ...
        masses = [mass * 1e-22 for mass in masses]
        ax0.plot(masses, ferric_per_fin, color=cols['N2'], linestyle='--', alpha=0.5)
        ax0.plot(masses, mixed_ferric_per, color=cols['N2'], alpha=0.5)
        ax11.plot(masses, ferric_bas_fin, color=cols['N2'], linestyle='--', alpha=0.5)
        ax11.plot(masses, mixed_ferric_bas, color=cols['N2'], alpha=0.5)
    ...
    plt.savefig(dir_path + '/figures/figure_9_comparison.pdf', dpi=200)
    # plt.show()
