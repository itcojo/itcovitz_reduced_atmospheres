...
def plot_figure_6() -> None:
    ...
    # with bug fix
    h2_fix, h2o_fix, co2_fix, n2_fix, co_fix, ch4_fix, pressures_fix = [], [], [], [], [], [], []
    h2_i_fix, h2o_i_fix, co2_i_fix, n2_i_fix, pressures_i_fix = [], [], [], [], []
    fo2_atmos_fix, fo2_bas_fix, fo2_per_fix, fe_bas_fix, fe_per_fix = [], [], [], [], []
    masses_per_fix, masses_bas_fix = [], []

    # convert from moles to column density
    fac = 1e-4 / (4. * np.pi * gC.r_earth ** 2.)

    # --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- ---
    # ORIGINAL VALUES (BEFORE BUG FIX)
    # --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- ---
    for m_imp in impactor_masses:
        ...
        file = dir_path + '/output/pre_erratum/m_imps/peridotite_3A_'
        ...
        # read data - basalt
        file = dir_path + '/output/pre_erratum/m_imps/basalt_3A_'
        with h5py.File(file + var_string + '.hdf5', 'r') as f:
    ...
    handles.append(lines.Line2D([0.], [0.], label='Atmos LTE', color='grey',
                                linestyle='-', marker='o'))
    handles.append(lines.Line2D([0.], [0.], label='Erratum', color='grey',
                                linestyle='-', marker='^'))

    ax0.legend(handles=handles, loc='lower left', ncol=5, fontsize=8,
               bbox_to_anchor=(-0.10, 1.01), columnspacing=0.9)
    ...
    ax0.plot(masses, h2, color=cols['H2'], marker='^', markersize=3, linestyle='')
    ax0.plot(masses, h2o, color=cols['H2O'], marker='^', markersize=3, linestyle='')
    ax1.plot(masses, p_h2o, color=cols['H2O'], marker='^', markersize=3, linestyle='')
    ax1.plot(masses, p_h2, color=cols['H2'], marker='^', markersize=3, linestyle='')
    ax2.plot(masses, fo2_atmos, color=cols['NH3'], marker='^', markersize=3, linestyle='')
    ax2.plot(masses, fo2_bas, color=cols['CO2'], marker='^', markersize=3, linestyle='')
    ax2.plot(masses, fo2_per, color=cols['CO2'], marker='^', markersize=3, linestyle='')
    idxs = [i for i, e in enumerate(fe_bas) if e != 0]
    ax3.plot([masses[i] for i in idxs], [fe_bas[i] for i in idxs],
             color='k', marker='^', markersize=3, linestyle='')
    ax3.plot(masses, fe_per, color='k', marker='^', markersize=3, linestyle='')

    plt.savefig(dir_path + '/figures/figure_6_comparison.pdf', dpi=200)
    # plt.show()
